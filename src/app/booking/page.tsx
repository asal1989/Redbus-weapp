...
import { Shield, ArrowLeft, Loader2, AlertCircle, Tag, CheckCircle, X } from 'lucide-react'
...
function BookingContent() {
  ...
  const finalPrice = booking ? Math.max(0, booking.totalPrice - (couponApplied?.discount ?? 0)) : 0

  async function applyCoupon() {
    if (!booking || !couponCode.trim()) return
    ...
    try {
      const res = await fetch('/api/coupon', {
        ...
        body: JSON.stringify({ code: couponCode.trim(), amount: booking.totalPrice }),
      })
      const data = await res.json()
      if (!data.ok) { setCouponError(data.error); return }
      setCouponApplied({ code: couponCode.toUpperCase(), discount: data.discount, description: data.coupon.description })
      setCouponCode('')
  ...
  return (
    <div className="bg-slate-50 min-h-screen">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-6 space-y-5">
        ...
        {/* Coupon code */}
        <div className="bg-white border border-slate-200 rounded-2xl p-5">
          <h2 className="font-bold text-slate-900 mb-3 flex items-center gap-2"><Tag className="w-4 h-4 text-accent-500" /> Coupon Code</h2>
          {couponApplied ? (
            <div className="flex items-center justify-between bg-green-50 border border-green-200 rounded-xl px-4 py-3">
              <div className="flex items-center gap-2">
                <CheckCircle className="w-4 h-4 text-green-600" />
                <div>
                  <span className="font-bold text-green-800 font-mono">{couponApplied.code}</span>
                  <span className="text-xs text-green-600 ml-2">{couponApplied.description}</span>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <span className="font-bold text-green-700">−{formatPrice(couponApplied.discount)}</span>
                <button onClick={() => setCouponApplied(null)} className="p-1 rounded-full hover:bg-green-200 transition-colors">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 border-2 border-slate-200 rounded-xl px-3 py-2.5 text-sm font-mono focus:outline-none focus:border-accent-500 uppercase"
              />
              <button
                onClick={applyCoupon}
                disabled={!couponCode || couponLoading}
                className="px-4 py-2.5 bg-accent-500 hover:bg-accent-600 disabled:opacity-50 text-white font-bold rounded-xl text-sm transition-colors"
              >
                {couponLoading ? '…' : 'Apply'}
              </button>
            </div>
          ...
          {!couponApplied && (
            <p className="mt-2 text-xs text-slate-400">Try: WELCOME10 · ARULJOTHI20 · FLAT50 · DIWALI100</p>
          )}
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between font-bold text-base text-slate-900 pt-2 border-t border-slate-100">
              ...
              <span className="text-accent-600">{formatPrice(finalPrice)}</span>
            </div>
          </div>
        </div>
